...
void screen_draw_box(uint32_t fInit, uint32_t cInit, uint32_t fSize,
  ...
  // f son las filas y c las columnas, fSize cantidad de filas y cSize cantidad de columnas
  ca(*p)[VIDEO_COLS] = (ca(*)[VIDEO_COLS])VIDEO;
  uint32_t f;
  uint32_t c;
...
void screen_draw_layout(void) {
  screen_draw_box(0, 0, VIDEO_FILS, VIDEO_COLS, ' ', C_BG_CYAN);
  char* gonzalo = "Gonzalo Azur Jesus Martinez";
  char* flor = "Florencia Lin Tseng";
  char* gabriel = "Victor Gabriel Fierro";
  print(gonzalo, (VIDEO_COLS/2) - 14, (VIDEO_FILS/2) - 1, C_BG_CYAN | C_FG_DARK_GREY);
  print(flor, (VIDEO_COLS/2) - 10, (VIDEO_FILS/2), C_BG_CYAN | C_FG_DARK_GREY);
  print(gabriel, (VIDEO_COLS/2) - 11, (VIDEO_FILS/2) + 1, C_BG_CYAN | C_FG_DARK_GREY);
}
